...
int func_tempo()                                                                                                        //Função para calcular o tempo
{
    ...
    if (func_qnt_tempo < 0 || func_qnt_tempo <= 0.1)
    {
        func_qnt_tempo = 0;
    }
}
    ...
    while (!WindowShouldClose()) {
            ...
            case MENU:
            {
                if(IsMouseButtonPressed(MOUSE_BUTTON_LEFT))                                                       //Detectando o botão esquerdo do mouse
                {
                    ...
                    else if(CheckCollisionPointRec(posMouse, botaoSair))                                      //Clicando no botão "SAIR"
                    {
                        ...
                        UnloadTexture(text_escritorio_2);
        ...
        switch (telaAtual) {                                                                                            //Fluxo de Desenho das Telas do Jogo
            ...
            case FASE:
            {
                ...
                {
                    DrawTexture(text_escritorio_1,6,6,WHITE);                    //Desenhando o ícone na atual posição do mouse
        ...
        if(IsMouseButtonDown(MOUSE_BUTTON_LEFT)){
            ...
            if(IsMouseButtonDown(MOUSE_BUTTON_LEFT))
            {
                DrawTexture(text_mouse_marca, spriteMouse.x - 10, spriteMouse.y - 10, WHITE);
            }
